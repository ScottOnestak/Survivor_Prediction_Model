...
for(i in seq(1:Weeks)){
  ...
  colnames(results) = paste('week',i,sep='')
...
for(i in seq(1:dim(current)[1])){
  if(is.na(clean[i,"EliminationWeek"])==F){
    ...
    for(j in seq(from=theWeek,to=Weeks,by=1)){
      clean[i,paste('week',j,sep='')] = NA
    }
  }
}

final = clean[,c(1:dim(clean)[2]-1)]
final[,c(3:length(final))] = NA
for(i in seq(1:dim(final)[1])){
  for(j in seq(from=3,to=dim(final)[2],by=1)){
    if(is.na(clean[i,j])==T){
      final[i,j] = ''
    } else {
      final[i,j] = clean[i,j]/sum(clean[,j],na.rm = T)
    }
  }
}
